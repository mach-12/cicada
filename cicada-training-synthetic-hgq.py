...
def get_student_targets(
    teacher: Model, gen: SyntheticRegionETGenerator, X: npt.NDArray
) -> data.Dataset:
...
def main(args) -> None:
    ...
    gen = SyntheticRegionETGenerator()
    X_train, X_val, X_test = gen.get_data_split()
    X_signal, _ = gen.get_benchmark(filter_acceptance=False)
    gen_train = gen.get_generator(X_train, X_train, 512, True)
    gen_val = gen.get_generator(X_val, X_val, 512)
    outlier_train = gen.get_data()
    outlier_val = gen.get_data()
    ...
    cicada_v1 = CicadaV1HGQ((252,)).get_model()
    ...
    cicada_v2 = CicadaV2HGQ((252,)).get_model()
